...
import {
  ...
  Section,
  MutationNofifyOpenTaskInDesktopArgs
...
const nofifyOpenTaskInDesktop = async (
  {
    taskId,
    jobId,
    publicationIndex,
  ...
  try {
    ...
    if(!candidateInformation || !candidateInformation?.data.firstName || !candidateInformation?.data.email)
      throw new Error('Error al enviar mailing de notificacion')

    const templateData = {
      companyLogo : jobInformation?.companyPublished?.photo || 'https://cdn.krowdy.com/images/magneto_2.png',
      ...
      subject     : 'Hola Mundo', // arreglar titulo de subject
      taskName    : taskInformation.task.categoryTask.pathContentForm! // arreglar titulo de la tarea
    }
